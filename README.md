...
## What's new: V2.1.0
gocesiumtiler V2.1.0 has been released introducing several important changes over V2.0.1 that should improve the output speed and quality. 

- Resolution is by default automatically selected, and the system includes an adaptive algorithm to tune it dynamically. It can still be defined manually, but by default it's adaptive.
- Tile sizes are now dynamic to reduce the number of tiles required to tile high density areas.
- Flat hierarchy: only 1 root tileset.json generated, and all binary files stored in a data folder. This will reduce the network calls and ease the portability.
- Possibility of defining the maximum number of points per tile: this will help achieve better control on the output size of the tiles.
...
## Changelog
##### Version 2.1.0
* Squash tileset.json: generate one single tileset.json for the entire point cloud.
* Flat binary file structure: all binary files are stored in a data folder, no more subdirectories.
* Dynamic resolution: the tool by default autoselects a value of resolution based on the point cloud size and can dynamically adapt it based on the point cloud.
* Adaptive tile sizes based on point density: tiles are now subdivided based on the relative density of points in the area.
* Support for defining a maximum number of points per tile.
* New default values for some parameters.
* **share** files for Proj are **no longer** included with the source code. These can be generated by building Proj either directly or via the `build.sh` or `build.ps1` scripts. After, if needed, move/copy the share folder into the `build` folder to the repository root.
...
```
   ...
   --crs value, -e value, --epsg value    String representing the input CRS. For example, EPSG:4326 or EPSG:28355+5773 or a generic Proj4 string. Bare numbers will be interpreted as EPSG codes. If empty the system will attempt to autodetect the CRS from the LAS metadata. In case of multiple LAS files, the CRS must be consistent else an error will be thrown.
   --resolution value, -r value           minimum resolution of the 3d tiles, in meters. approximately represets the maximum sampling distance between any two points at the lowest level of detail. Zero or negative means automatic estimation of resolution (default: 0)
   ...
   --min-points-per-tile value, -m value  minimum number of points to enforce in each 3D tile. Might be ignored when the parent has not enough capacity to store the child points (see max-points-per-tile) (default: 10000)
   --max-points-per-tile value, -M value  maximum number of points a tile can contain. If a tile contains more points, a subsampling strategy is applied. 0 means no limit. (default: 200000)
   ...
   --version value, -v value              sets the version of the tileset to generate. Could be either 1.0 or 1.1 (default: "1.0")   
   --help, -h                             show help
```
...
2. An octree cell is created. Every cell stores N points (variable). A cell also has a grid spacing property. The root node has a spacing set to the
 provided resolution. 
3. The points are traversed. Each point will fall into one of the cells (voxels) the space has been divided by the given grid spacing. If the point is 
